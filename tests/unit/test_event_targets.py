...
def test_fixture_event_targets_are_built_without_skips() -> None:
    ...
    assert result.events["is_macro_blackout"].sum() == 1
...
def test_macro_blackout_uses_received_timestamp_and_config_window() -> None:
    ...
    bars = load_market_csv(config.paths.market_fixture)
    result = build_event_dataset(posts, bars, config)
    blackout = result.events[result.events["is_macro_blackout"]]

    assert blackout["post_id"].tolist() == ["fixture-006"]
    assert blackout.iloc[0]["nearest_macro_event_id"] == "fixture-fed-speaker"
    assert blackout.iloc[0]["nearest_macro_event_type"] == "fed_speaker"
    assert blackout.iloc[0]["minutes_to_nearest_macro_event"] == 9.4167
